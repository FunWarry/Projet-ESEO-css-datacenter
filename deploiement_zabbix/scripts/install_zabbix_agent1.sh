...
if [ -f /etc/zabbix/zabbix_agentd.conf ]; then
    ...
    sudo sed -i 's/^Server=127.0.0.1/Server=192.168.XX.XX/' /etc/zabbix/zabbix_agentd.conf ##IP du serveur zabbix
    sudo sed -i 's/^# ListenIP=0.0.0.0/ListenIP=0.0.0.0/' /etc/zabbix/zabbix_agentd.conf
    sudo sed -i 's/^ServerActive=127.0.0.1/ServerActive=192.168.XX.XX/' /etc/zabbix/zabbix_agentd.conf ##IP du serveur zabbix
    sudo sed -i 's/^Hostname=Zabbix server/Hostname=XXXXXXXXXXXXX/' /etc/zabbix/zabbix_agentd.conf ##hostname de l'eclave zabbix
